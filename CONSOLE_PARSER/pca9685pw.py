...
class Pca9685pw:
    ...
    def __init__(self, defaultPins=8, defaultBus=8, defaultAddress=0b1000000):
        self.defaultBus = 8 
    ...
    def fadeToState(self,ledNum,goal):
        steps = 100
        totalTime = 2.0
        startState = self.getPercent(ledNum)
        endState = (goal/255.0*100)
        StateDiff = endState - startState
        #print 'start',startState
        #print 'end',endState
        for i in range(0,steps):
            self.setPercent(ledNum,startState+(StateDiff/steps)*i)
            time.sleep(totalTime/steps)
