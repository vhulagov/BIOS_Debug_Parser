...
//#define PCI_DEVICE_ID_INTEL_BROADWELL_BR          0x6fae
//#define PCI_DEVICE_ID_INTEL_BROADWELL_BR          0x6ffc

#define PCI_DEVICE_ID_INTEL_SBRIDGE_IMC_TA      0x3ca8  /* 15.0 */
#define PCI_DEVICE_ID_INTEL_BROADWELL_IMC_TA      0x6fa8 /* 13.0 */
...
static int imc_init_channel(struct imc_priv *priv, int i, int socket)
{
	...
	ch->can_write = 1; //!(val & SMBCNTL_DIS_WRT);
					      ...
					      unsigned int devfn, u16 devid)
{
	...
	printk(KERN_WARNING "imc_get_related_device...\n");
	dev = pci_get_slot(bus, devfn);
	//struct pci_dev *dev = pci_get_domain_bus_and_slot(bus, devfn);
	printk(KERN_WARNING "...done\n");

	if (!dev) {
		printk(KERN_WARNING "dev is empty!\n");
		return NULL;
	}
	if (dev->vendor != PCI_VENDOR_ID_INTEL || dev->device != devid) {
		printk(KERN_WARNING "dev is not SAD!\n");
		pci_dev_put(dev);
		return NULL;
	}
	if (dev) {
		printk(KERN_WARNING "dev is not empty: %02x:%02x, we search for %02x:%02x\n",
			dev->vendor, dev->device, PCI_VENDOR_ID_INTEL, devid);
	}
	
	printk(KERN_WARNING "SAD device founded\n");
	return dev;
}
...
//		return -ENODEV;
	 ...
	 */
	//sad = imc_get_related_device(dev->bus, PCI_DEVFN(0x12, 0), 0x6fa0);
	sad = imc_get_related_device(dev->bus, PCI_DEVFN(0x13, 0), 0x6fa8);
	if (!sad)
		return -ENODEV;
...
static const struct pci_device_id imc_ids[] = {
	...
	{ PCI_DEVICE(PCI_VENDOR_ID_INTEL, PCI_DEVICE_ID_INTEL_BROADWELL_IMC_TA) },
	{ 0, }
};
